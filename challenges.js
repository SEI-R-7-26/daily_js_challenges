...
function reverseUpcaseString(string) {
  ...
  stringArray.forEach((char) => {
    newStrArray.unshift(char.toUpperCase())
  })
...
function charCount(string) {
  ...
  stringArray.forEach((item) => {
    if (!countedCharacters[item]) {
      countedCharacters[item] = 1
    } else {
      countedCharacters[item] = countedCharacters[item] + 1
    }
  })
...
function formatWithPadding(n, char, length) {
  if (n.toString().length >= length) {
    ...
    for (let i = n.toString().length; i < length; i++) {
...
function mergeObjects(obj1, obj2) {
  if (Object.keys(obj1).length) {
    ...
    for (let i = 0; i < arguments.length; i++) {
      mergedObject = { ...mergedObject, ...arguments[i] }
    }
    return mergedObject
  }
  return obj1
...
function findHighestPriced(arr) {
  ...
  const highestIndex = arr.indexOf(sorted[0])
...
function toCamelCase(string) {
  let strArray = string.split(/[_-]/g).map((str, i) => {
    if (i > 0) {
