...
function reverseUpcaseString(str) {
  ...
  for (let i = 0; i < str.length; i++) {
    result = str.charAt(i).toUpperCase() + result
  }
  return result
}
/*-----------------------------------------------------------------
...
function intersection(arr1, arr2) {
  ...
  let ar2 = [...arr2]
  arr1.forEach((val) => {
    let indx = ar2.indexOf(val)
    if (indx > -1) result.push(ar2.splice(indx, 1)[0])
  })
  return result
}
