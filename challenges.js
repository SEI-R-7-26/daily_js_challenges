...
function addList() {
  ...
  let sum = 0
  sum = nums.reduce((accumulator, num) => {
...
function computeRemainder(n1, n2) {
  ...
  if (n2 === 0) {
    ...
    solution = n1 - Math.floor(n1 / n2) * n2
  }
  return solution
}
/*-----------------------------------------------------------------
...
function removeEnds(string) {
  removeFirstChar = string.slice(1)
  removeLastChar = removeFirstChar.slice(0, removeFirstChar.length - 1)
...
function intersection(arr1, arr2) {
  ...
  for (let i = 0; i < arr1.length; i++) {
    for (let j = 0; j < copyArr.length; j++) {
      if (arr1[i] === arr2[j]) {
        newArr.push(copyArr.splice(j, 1)[0])
        break
      }
    }
  }
  ...
  for (let i = 0; i < string.length; i++) {
    if (
      ...
      if (string.charAt(i) === ')' && stack[stack.length - 1] === '(') {
        stack.pop()
      } else if (string.charAt(i) === ']' && stack[stack.length - 1] === '[') {
        stack.pop()
      } else if (string.charAt(i) === '}' && stack[stack.length - 1] === '{') {
        stack.pop()
      }
...
function getNumForIP(bits) {
  let chunkOne = parseInt(bits.slice(0, bits.indexOf('.')))
  bits = bits.slice(chunkOne.toString().length + 1)
  let chunkTwo = parseInt(bits.slice(0, bits.indexOf('.')))
  bits = bits.slice(chunkTwo.toString().length + 1)
  let chunkThree = parseInt(bits.slice(0, bits.indexOf('.')))
  bits = bits.slice(chunkThree.toString().length + 1)
  let chunkFour = parseInt(bits)

  return (
    chunkFour +
    chunkThree * 256 +
    chunkTwo * Math.pow(256, 2) +
    chunkOne * Math.pow(256, 3)
  )
}
/*-----------------------------------------------------------------
