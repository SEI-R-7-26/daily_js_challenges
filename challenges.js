...
function isPalindrome(string) {
  ...
  for (let i = 0; i < string.length; i++) {
    if (string.charAt(i) != ' ') {
      reversed = string.charAt(i).toLowerCase() + reversed
      nospace = nospace + string.charAt(i).toLowerCase()
    }
  }
  if (nospace === reversed) {
    return true
...
function mumble(string) {
  answer = ''
  for (let i = 0; i < string.length; i++) {
    answer = answer + string.charAt(i).repeat(i + 1)
    i !== string.length - 1 && (answer = answer + '-')
  }
  return answer
}
    ...
    if (Array.isArray(arr[i])) {
      deeper = (levels) => {
        for (let j = 0; j < levels.length; j++) {
          if (Array.isArray(levels[j])) {
            deeper(levels[j])
          } else {
            answer.push(levels[j])
          }
        }
      }
      deeper(arr[i])
