...
function computeRemainder(n1, n2) {
  ...
  let quotientWhole = parseInt(n1 / n2)
  return n1 - quotientWhole * n2
}
/*-----------------------------------------------------------------
Challenge: 06-range
...
function range(n1, n2) {
  if (n2 < n1) return 'First argument must be less than second'
  let rangeArr = []
  for (let start = n1; start < n2; start++) {
    rangeArr.push(start)
  }
  return rangeArr
}
...
function reverseUpcaseString(string) {
  let newString = string.split('').reverse().join('').toUpperCase()
  return newString
}
...
function removeEnds(string) {
  if (string.length < 3) {
    ...
    let newString = string.substr(1).slice(0, -1)
    return newString
  }
}
/*-----------------------------------------------------------------
...
function isPalindrome(string) {
  let fixedString = string.toLowerCase().replace(/ /g, '')
  if (fixedString.length <= 1) {
    return true
  } else {
    for (let i = 0; i < Math.floor(fixedString.length / 2); i++) {
      if (
        fixedString.charAt(i) !== fixedString.charAt(fixedString.length - i - 1)
      )
        return false
    }
    return true
...
function fromPairs(arr) {
  let object = {}
  arr.forEach(function (ivArr) {
    object[ivArr[0]] = ivArr[1]
  })
  return object
}
/*-----------------------------------------------------------------
...
function reduceArray(arr, acc, value) {
  ...
  arr.forEach(function (el, idx) {
    initValue = acc(initValue, el, idx)
  })
  return initValue
}
/*-----------------------------------------------------------------
...
function balancedBrackets(string) {
  // test edge cases > think of built in functions > start with inefficiency > get comfortable with built in functions > for ... of >
  let brackets = '[]{}()'
  let stack = []
...
function isWinningTicket(arr) {
  let winner = true
  for (x in arr) {
    let numToChar = String.fromCharCode(arr[x][1])
    !arr[x][0].includes(numToChar) ? (winner = false) : null

    // arr[x][0].includes(String.fromCharCode[x][1]) ? true : false
  }
  return winner
}
/*-----------------------------------------------------------------
