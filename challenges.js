...
function flatten(arr) {
  let newArr = []
  arr.forEach(function (wow) {
    if (Array.isArray(wow)) {
      newArr = newArr.concat(flatten(wow))
    } else {
      newArr.push(wow)
    }
  })
...
function primeFactors(n) {
  ...
  let optimusPrime = 2
  while (!primes(n)) {
    if (Number.isInteger(n / optimusPrime)) {
      factors.push(optimusPrime)
      n = n / optimusPrime
    } else {
      optimusPrime++
      while (!primes(optimusPrime)) optimusPrime++
    }
  }
  factors.push(n)
  return factors
